...
import { fetchJewelleryByPrice } from '../api/categoryApi';
...
const RANGE_MAP = {
  ...
  '2000-3000': { min: 2000, max: 3000, label: '$2,000 - $3,000' },
  '3000-5000': { min: 3000, max: 5000, label: '$3,000 - $5,000' },
  '5000-up':   { min: 5000, max: null, label: 'Above $5,000'    },
};
...
export default function PriceFilteredPage() {
  ...
  const { min: minPrice, max: maxPrice, label } = RANGE_MAP[rangeKey] || { min: undefined, max: undefined, label: 'Jewellery by Price' };
  ...
  useEffect(() => { setVisibleCount(20); }, [metalType, rangeKey]);
  ...
  useEffect(() => {
    ...
    return () => { stale = true; };
  }, [metalType, rangeKey]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="cat-page">
      ...
      {!loading && !error && sorted.length === 0 && (
        <div className="cat-page__error">No products found in this price range.</div>
      )}
